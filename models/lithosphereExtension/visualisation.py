...
def plotMarkerFields_Lithology(xsize, ysize, markers, grid, ntstp, t_curr):
    ...
    axs.set(ylabel='y (m)', xlabel ='x (m)')                                         # labels, limits                        
...
def plotMarkerFields(xsize, ysize, markers, grid, ntstp, t_curr):
    ...
    im = axs[0].imshow(mark_sigmaii, origin='upper', aspect='auto', extent=[0,4e5,3e5,0])             
    fig.colorbar(im, ax=axs[0],pad=0.0)                                                 # display colorbar
    axs[0].set_title('$\\sigma_{ii}$ (Pa)')                                                       # set plot title
    axs[0].set(ylabel='y (m)')                      # labels, limits
    ...
    im = axs[1].imshow(mark_sigmaxx, origin='upper', aspect='auto', extent=[0,4e5,3e5,0])                     # extent=[0,xsize,ysize,0]
    fig.colorbar(im, ax=axs[1],pad=0.0)                                                 # display colorbar
    axs[1].set_title('$\\sigma_{xx}$ (Pa)')                                                          # set plot title
    axs[1].set(ylabel = 'y (m)')                   # labels, limits
    ...
    im = axs[2].imshow(mark_sigmaxy, origin='upper', aspect='auto', extent=[0,4e5,3e5,0])                     # extent=[0,xsize,ysize,0]
    fig.colorbar(im, ax=axs[2],pad=0.0)                                                 # display colorbar
    axs[2].set_title('$\\sigma_{xy}$ (Pa)')                                              # set plot title
    axs[2].set(xlabel='x (m)', ylabel = 'y (m)')   # labels, limits
...
def plotMarkerFields2(xsize, ysize, markers, grid, ntstp, t_curr):
    ...
    # plot the normal strain rate component  as colormap
    im = axs[0].imshow(mark_epsxx, origin='upper', aspect='auto', extent=[0,4e5,3e5,0], vmin=-4e-14, vmax=4e-14) 
    fig.colorbar(im, ax=axs[0],pad=0.0)                                                 # display colorbar
    ...
    im = axs[1].imshow(mark_epsxy, origin='upper', aspect='auto', extent=[0,4e5,3e5,0], vmin=-4e-14, vmax=4e-14)
    fig.colorbar(im, ax=axs[1],pad=0.0)                                                 # display colorbar
    ...
    # plot normal stress components
    im = axs[2].imshow(mark_epsii, origin='upper', aspect='auto', extent=[0,4e5,3e5,0], vmin=-4e-14, vmax=4e-14)
    fig.colorbar(im, ax=axs[2],pad=0.0)                                                 # display colorbar
    axs[2].set_title('$\\dot \\epsilon_{ii}$ (1/s)')			      # set plot title
    axs.[2].set(ylabel = 'y (m')                                              # labels, limits
    ...
    im = axs[3].imshow(np.log10(mark_gii), origin='upper', aspect='auto', extent=[0,4e5,3e5,0], vmin=-2, vmax=2)
    fig.colorbar(im, ax=axs[3],pad=0.0)                                                 # display colorbar
    axs[3].set_title(' Total strain (log10)')                                              # set plot title
    axs[3].set(xlabel='x (m)', ylabel = 'y (m)') 
...
def get_marker_fields_vis(xsize, ysize, markers, grid):
    ...
    xres = grid.xnum
    yres = grid.ynum #int(ysize/xsize*xres) + 1
