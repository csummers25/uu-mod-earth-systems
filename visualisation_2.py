...
def plotAVar(grid, vxb, vyb, L_x, L_y, ntstp, t_curr):
    ...
    im = axs.pcolor(X, Y, grid.rho, shading='nearest', vmin=2200, vmax=3500)
    ...
    qu = axs.quiver(X[::step, ::step], Y[::step, ::step], vxb[:yres,:][::step, ::step], np.flip(-vyb[:,:xres],0)[::step, ::step]) 
    axs.invert_yaxis()
...
def plotSeveralVars(grid, vxb, vyb, L_x, L_y, ntstp, t_curr):
    ...
    axs[0].set_title('Density')     # set plot title
    ...
    axs[1].set_title('Viscosity')     # set plot title
    # Add temperature contours
    ...
    axs[2].set_title('Pressure') #set plot title
    # Add temperature contours
...
def plotMarkerFields(xsize, ysize, markers, grid, ntstp, t_curr):
    ...
    mark_com, mark_gii, mark_sigmaxy = get_marker_fields_vis(xsize, ysize, markers, grid)
    ...
    im = axs[2].imshow(mark_sigmaxy, origin='upper', aspect='auto', extent=[0,xsize,ysize,0])
    fig.colorbar(im, ax=axs[2],pad=0.0) # display colorbar
    axs[2].set_title('Shear stress')     # set plot title
        ...
        for m10 in range(m1min, m1max):
            for m20 in range(m2min, m2max):
                ...
                if (dd<mark_dis[m20, m10]):
                    ...
                    mark_dis[m20, m10] = dd
                    mark_sigmaxy[m20, m10] = markers.epsii[m]

    return mark_com, mark_gii, mark_sigmaxy
